...
record IsOrderedField
  ...
  *-sign-1 : {x y : A} → F x → F y
                  → x ≤ 0# → y ≤ 0# → 0# ≤ (x * y)
  *-sign-1 {x} {y} x' y' x≤0 y≤0
    = ≤-begin
      0#
  ...
  *-sign-2 : {x y : A} → F x → F y
                  → x ≤ 0# → y ≥ 0# → (x * y) ≤ 0#
  *-sign-2 {x} {y} x' y' x≤0 y≥0
    = ≤-begin
      x * y
    ...
    where -xy≥0 = ≤-begin
                  0#
                ≤≈⟨ 0' , *-sign-1 x' (-' y') x≤0 (≥-‿reverse y' y≥0) ⟩
                  x * (- y)
                ≈⟨ x' *' (-' y') , x' *-comm (-' y') ⟩
                  (- y) * x
                ≈⟨ (-' y') *' x' , -‿assoc y' x' ⟩
                ...
                ≈⟨ -' (y' *' x') , -‿cong (y' *' x') (x' *' y') (y' *-comm x') ⟩
                  - (x * y)
                ∎⟨ -' (x' *' y') ⟩
  *-sign-3 : {x y : A} → F x → F y
                  → x ≥ 0# → y ≤ 0# → (x * y) ≤ 0#
  *-sign-3 {x} {y} x' y' x≥0 y≤0
    = ≤-begin
      x * y
    ≈≤⟨ x' *' y' , x' *-comm y' ⟩
      y * x
    ≤≈⟨ y' *' x' , (*-sign-2 y' x' y≤0 x≥0) ⟩
      0#
    ∎⟨ 0' ⟩
  *-sign-4 : {x y : A} → F x → F y
                      → x ≥ 0# → y ≥ 0# → (x * y) ≥ 0#
  *-sign-4 {x} {y} x' y' x≥0 y≥0
      = ≤-begin
        0#
      ≈≤˘⟨ 0' , 0-zeroˡ y' ⟩
        0# * y
  ...
  prop-1-1-2-xiv {x} x' = [ (λ 0≤x → ≤*-comp x' x' 0≤x 0≤x) , (λ x≤0 → *-sign-1 x' x' x≤0 x≤0) ] (sign x')
  ...
  prop-1-1-5-iii : {x y : A} → (x' : F x) → (y' : F y)
                  → ∣ x' *' y' ∣ ≈ (∣ x' ∣ * ∣ y' ∣)
  prop-1-1-5-iii {x} {y} x' y' with sign x' | sign y' | sign (x' *' y')
  prop-1-1-5-iii {x} {y} x' y' | (inj₁ x≥0) | (inj₁ y≥0) | (inj₁ xy≥0) = refl (x' *' y')
  prop-1-1-5-iii {x} {y} x' y' | (inj₁ x≥0) | (inj₂ y≤0) | (inj₁ xy≥0)
    = begin
      x * y
    ≈⟨ x' *' y' , xy≈0 ⟩
      0#
    ...
    ≈⟨ (-' y') *' x' , (-' y') *-comm x' ⟩
      x * (- y)
    ∎⟨ x' *' (-' y') ⟩
    where xy≤0 = *-sign-3 x' y' x≥0 y≤0
          xy≈0 = ≤-anti-sym (x' *' y') 0' xy≤0 xy≥0
  prop-1-1-5-iii {x} {y} x' y' | (inj₁ x≥0) | (inj₁ y≥0) | (inj₂ xy≤0)
    = begin
      - (x * y)
    ≈⟨ -' (x' *' y') , -‿cong (x' *' y') 0' xy≈0 ⟩
      - 0#
    ≈⟨ -' 0' , negativeZero ⟩
      0#
    ≈˘⟨ 0' , xy≈0 ⟩
  ...
  prop-1-1-5-iii {x} {y} x' y' | (inj₂ x≤0) | (inj₁ y≥0) | (inj₁ xy≥0)
    ...
    ≈˘⟨ -' (x' *' y') , -‿assoc x' y' ⟩
      (- x * y)
    ∎⟨ -' x' *' y' ⟩
    where xy≤0 = *-sign-2 x' y' x≤0 y≥0
          xy≈0 = ≤-anti-sym (x' *' y') 0' xy≤0 xy≥0
  prop-1-1-5-iii {x} {y} x' y' | (inj₂ x≤0) | (inj₂ y≤0) | (inj₁ xy≥0) = ≈-sym (-' x' *' -' y') (x' *' y') (*-‿cancel x' y')
  prop-1-1-5-iii {x} {y} x' y' | (inj₂ x≤0) | (inj₁ y≥0) | (inj₂ xy≤0) = ≈-sym (-' x' *' y') (-' (x' *' y')) (-‿assoc x' y')
  prop-1-1-5-iii {x} {y} x' y' | (inj₂ x≤0) | (inj₂ y≤0) | (inj₂ xy≤0)
    = begin
      - (x * y)
    ≈⟨ -' (x' *' y') , -‿cong (x' *' y') 0' xy≈0 ⟩
    ...
    ≈˘⟨ x' *' y' , *-‿cancel x' y' ⟩
      - x * - y
    ∎⟨ -' x' *' -' y' ⟩
    where xy≥0 = *-sign-1 x' y' x≤0 y≤0
          xy≈0 = ≤-anti-sym (x' *' y') 0' xy≤0 xy≥0


  lemma-triIneq-1-1 : {x y : A} → (x' : F x) → (y' : F y)
                    → x ≤ y → - x ≤ y → (∣ x' ∣ ≤ y)
  lemma-triIneq-1-1 x' y' x≤y -x≤y with sign x'
  lemma-triIneq-1-1 x' y' x≤y -x≤y | (inj₁ x≥0) = x≤y
  lemma-triIneq-1-1 x' y' x≤y -x≤y | (inj₂ x≤0) = -x≤y


  lemma-triIneq-1-2-1 : {x y : A} → (x' : F x) → (y' : F y)
                    → ∣ x' ∣ ≤ y
                    → (x ≤ y) × (- x ≤ y)
  lemma-triIneq-1-2-1 x' y' x≤y with sign x'
  lemma-triIneq-1-2-1 x' y' x≤y | (inj₁ x≥0) = (x≤y , ≤-trans -x' 0' y' -x≤0 0≤y)
    where -x' = -' x'
          -x≤0 = ≥-‿reverse x' x≥0
          0≤y = ≤-trans 0' x' y' x≥0 x≤y
  lemma-triIneq-1-2-1 x' y' -x≤y | (inj₂ x≤0) = (≤-trans x' 0' y' x≤0 0≤y , -x≤y)
    where -x' = -' x'
          0≤y = ≤-trans 0' -x' y' (≤-‿reverse x' x≤0) -x≤y
  postulate
    lemma-triIneq-1-2-2 : {x y : A} → (x' : F x) → (y' : F y)
                      → ∣ x' ∣ < y
                      → (x < y) × (- x < y)
    -- lemma-triIneq-1-2-2 x' y' (∣x∣≤y , ∣x∣≉y)
    --   = ( (proj₁ (lemma-triIneq-1-2-1 x' y' ∣x∣≤y) , {!   !})
    --       , (proj₂ (lemma-triIneq-1-2-1 x' y' ∣x∣≤y) , {!   !}) )
  ...
  lemma-triIneq-2-1 : {x y : A} → (x' : F x) → (y' : F y)
                    → x ≤ y → - y ≤ x → ∣ x' ∣ ≤ y
  lemma-triIneq-2-1 x' y' x≤y -y≤x with sign x'
  lemma-triIneq-2-1 x' y' x≤y -y≤x | (inj₁ x≥0) = x≤y
  lemma-triIneq-2-1 {x} {y} x' y' x≤y -y≤x | (inj₂ x≤0)
    = ≤-begin
      - x
    ≤≈⟨ -' x' , ≤-‿comp (-' y') x' -y≤x ⟩
      - (- y)
  ...
  postulate
    lemma-triIneq-2-1' : {x y : A} → (x' : F x) → (y' : F y)
                      → x < y → - y < x → ∣ x' ∣ < y
    -- lemma-triIneq-2-1' x' y' (x≤y , x≉y) (-y≤x , -y≉x) = ( ∣x∣≤y , {!   !} )
    --   where ∣x∣≤y = lemma-triIneq-2-1 x' y' x≤y -y≤x
  ...
  --                               )
  --
  lemma-triIneq-3 : {x : A} → (x' : F x)
                    → x ≤ ∣ x' ∣
  lemma-triIneq-3 x' with sign x'
  ...
  lemma-triIneq-3 {x} x' | (inj₂ x≤0) = ≤-begin
                                      ...
                                      ≤⟨ 0' , ≤-‿reverse x' x≤0 ⟩
                                        - x
                                      ≤-∎⟨ -' x' ⟩
  lemma-triIneq-4 : {x : A} → (x' : F x) → - x ≤ ∣ x' ∣
  lemma-triIneq-4 x' with sign x'
  ...
  ∣∣-triangleIneq : {x y : A} → (x' : F x) → (y' : F y)
                  → ∣ x' +' y' ∣ ≤ (∣ x' ∣ + ∣ y' ∣)
  ∣∣-triangleIneq {x} {y} x' y'
    = lemma-triIneq-1-1 (x' +' y') (∣ x' ∣' +' ∣ y' ∣') x+y≤∣x∣+∣y∣ -x-y≤∣x∣+∣y∣
      where x+y≤∣x∣+∣y∣ = ≤-begin
                          x + y
                        ≤⟨ x' +' y' , ≤+-compˡ x' ∣ x' ∣' y' (lemma-triIneq-3 x') ⟩
                          ∣ x' ∣ + y
                        ≤⟨ ∣ x' ∣' +' y' , ≤+-compʳ y' ∣ y' ∣' ∣ x' ∣' (lemma-triIneq-3 y') ⟩
                          ∣ x' ∣ + ∣ y' ∣
                        ≤-∎⟨ ∣ x' ∣' +' ∣ y' ∣' ⟩
            -x-y≤∣x∣+∣y∣ = ≤-begin
                          - (x + y)
                        ≈≤⟨ -' (x' +' y') , -‿distrib x' y' ⟩
                          (- x) + (- y)
                        ≤⟨ (-' x') +' (-' y') , ≤+-compˡ (-' x') ∣ x' ∣' (-' y') (lemma-triIneq-4 x') ⟩
                          ∣ x' ∣ + (- y)
                        ≤⟨ ∣ x' ∣' +' (-' y') , ≤+-compʳ (-' y') ∣ y' ∣' ∣ x' ∣' (lemma-triIneq-4 y') ⟩
                          ∣ x' ∣ + ∣ y' ∣
                        ≤-∎⟨ ∣ x' ∣' +' ∣ y' ∣' ⟩
  ...
  prop-1-1-6-i : {x y : A} → (x' : F x) → (y' : F y)
                → 0# ≤ d (x' , y')
  prop-1-1-6-i x' y' = prop-1-1-5-i (x' +' -' y')


  lemma-1-1-6-ii-1 : {x y : A} → (x' : F x) → (y' : F y)
                    → (x + (- y)) ≈ 0#
                    → x ≈ y
  lemma-1-1-6-ii-1 {x} {y} x' y' x-y≈0
    = begin
      x
    ≈⟨ x' , -‿uniqueˡ x' (-' y') x-y≈0 ⟩
      - (- y)
    ≈⟨ -' (-' y') , -‿doubleInverse y' ⟩
      y
    ∎⟨ y' ⟩
  ...
  prop-1-1-6-ii-1 : {x y : A} → (x' : F x) → (y' : F y)
                → d (x' , y') ≈ 0#
                → x ≈ y
  prop-1-1-6-ii-1 {x} {y} x' y' ∣x-y∣≈0
    = lemma-1-1-6-ii-1 x' y' x-y≈0
    where x-y≈0 = (proj₁ (prop-1-1-5-ii (x' +' -' y'))) ∣x-y∣≈0

  lemma-1-1-6-ii-2 : {x y : A} → (x' : F x) → (y' : F y)
                    → x ≈ y
                    → (x + (- y)) ≈ 0#
  lemma-1-1-6-ii-2 {x} {y} x' y' x≈y
    = begin
      x + (- y)
    ≈⟨ x' +' (-' y') , +-congˡ x' y' (-' y') x≈y ⟩
      y + (- y)
  ...
  prop-1-1-6-ii-2 : {x y : A} → (x' : F x) → (y' : F y)
                → x ≈ y
                → d (x' , y') ≈ 0#
  prop-1-1-6-ii-2 {x} {y} x' y' x≈y
    = (proj₂ (prop-1-1-5-ii (x' +' -' y'))) x-y≈0
    where x-y≈0 = lemma-1-1-6-ii-2 x' y' x≈y


  prop-1-1-6-ii : {x y : A} → (x' : F x) → (y' : F y)
                  → (d (x' , y') ≈ 0#) ↔ (x ≈ y)
  prop-1-1-6-ii x' y'
    = (prop-1-1-6-ii-1 x' y' , prop-1-1-6-ii-2 x' y')

  lemma-1-1-6-iii-1 : {x y : A} → (x' : F x) → (y' : F y)
                    → (x + (- y)) ≥ 0#
                    → x ≥ y
  lemma-1-1-6-iii-1 {x} {y} x' y' x-y≥0
    = ≤-begin
      y
    ≈≤˘⟨ y' , 0-identityˡ y' ⟩
      0# + y
    ≤≈⟨ 0' +' y' , ≤+-compˡ 0' (x' +' (-' y')) y' x-y≥0 ⟩
      (x + (- y)) + y
    ≈⟨ (x' +' (-' y')) +' y' , +-assoc x' (-' y') y' ⟩
      x + ((- y) + y)
    ≈⟨ x' +' ((-' y') +' y') , +-congʳ ((-' y') +' y') 0' x' (-‿inverseˡ y') ⟩
      x + 0#
    ≈⟨ x' +' 0' , x' 0-identityʳ ⟩
      x
    ∎⟨ x' ⟩

  lemma-1-1-6-iii-2 : {x y : A} → (x' : F x) → (y' : F y)
                    → (x + (- y)) ≤ 0#
                    → x ≤ y
  lemma-1-1-6-iii-2 {x} {y} x' y' x-y≤0
    = ≤-begin
      x
    ≈≤˘⟨ x' , x' 0-identityʳ ⟩
      x + 0#
    ≈≤˘⟨ x' +' 0' , +-congʳ ((-' y') +' y') 0' x' (-‿inverseˡ y') ⟩
      x + ((- y) + y)
    ≈≤˘⟨ x' +' ((-' y') +' y') , +-assoc x' (-' y') y' ⟩
      (x + (- y)) + y
    ≤≈⟨ (x' +' (-' y')) +' y' , ≤+-compˡ (x' +' (-' y')) 0' y' x-y≤0 ⟩
      0# + y
    ≈⟨ 0' +' y' , 0-identityˡ y' ⟩
      y
    ∎⟨ y' ⟩
  ...
  prop-1-1-6-iii : {x y : A} → (x' : F x) → (y' : F y)
                  → d (x' , y') ≈ d (y' , x')
  prop-1-1-6-iii {x} {y} x' y' with sign (x' +' (-' y')) | sign (y' +' (-' x'))
  prop-1-1-6-iii {x} {y} x' y' | inj₁ x-y≥0 | inj₁ y-x≥0
    = begin
      x + (- y)
    ≈⟨ x' +' -' y' , lemma-1-1-6-ii-2 x' y' x≈y ⟩
      0#
    ≈˘⟨ 0' , lemma-1-1-6-ii-2 y' x' (≈-sym x' y' x≈y) ⟩
      y + (- x)
    ∎⟨ y' +' -' x' ⟩
    where y≤x = lemma-1-1-6-iii-1 x' y' x-y≥0
          x≤y = lemma-1-1-6-iii-1 y' x' y-x≥0
          x≈y = ≤-anti-sym x' y' x≤y y≤x
  prop-1-1-6-iii {x} {y} x' y' | inj₁ x-y≥0 | inj₂ y-x≤0
    = begin
      x + (- y)
    ≈⟨ x' +' (-' y') , x' +-comm (-' y') ⟩
  ...
  prop-1-1-6-iii {x} {y} x' y' | inj₂ x-y≤0 | inj₁ y-x≥0
    ...
    ∎⟨ y' +' (-' x') ⟩
  prop-1-1-6-iii {x} {y} x' y' | inj₂ x-y≤0 | inj₂ y-x≤0
    = -‿cong (x' +' -' y') (y' +' -' x') x-y≈y-x
    where x≤y = lemma-1-1-6-iii-2 x' y' x-y≤0
          y≤x = lemma-1-1-6-iii-2 y' x' y-x≤0
          x≈y = ≤-anti-sym x' y' x≤y y≤x
          x-y≈y-x = begin
                    x + (- y)
                  ≈⟨ x' +' -' y' , lemma-1-1-6-ii-2 x' y' x≈y ⟩
  ...
  d-triangleIneq : {x y z : A} → (x' : F x) → (y' : F y)→ (z' : F z)
                 → d (x' , y') ≤ (d (x' , z') + d (z' , y'))
  d-triangleIneq x' y' z'
    = ≤-begin
      d (x' , y')
    ≈≤⟨ d' x' y' , ∣∣-cong (x' +' (-' y')) ((x' +' (-' z')) +' (z' +' (-' y'))) x-y≈x-z+z-y ⟩
      ∣ (x' +' (-' z')) +' (z' +' (-' y')) ∣
    ≤⟨ ∣ ((x' +' (-' z')) +' (z' +' (-' y'))) ∣' , ∣∣-triangleIneq (x' +' (-' z')) (z' +' (-' y')) ⟩
      d (x' , z') + d (z' , y')
    ≤-∎⟨ (d' x' z') +' (d' z' y') ⟩
  ...
  sandwich-lemma {x} {y} {z} {x'} {y'} {z'} {a} {a'}
    ...
    = (N , n≥N→∣zn-a∣<ε)
      where Nx₁ = proj₁ (limx≈a ε∈F ε≥0)
            ...
            n≥N→-xn+a<ε = λ {n} → λ n≥N → proj₂ (lemma-triIneq-1-2-2 (x' n +' (-' a')) ε∈F (n≥N→∣xn-a∣<ε (n≥N→n≥Nx₁ n≥N)))
            n≥N→-ε<xn-a = λ {n : ℕ} → λ n≥N → <-‿moveˡ (x' n +' (-' a')) ε∈F (n≥N→-xn+a<ε n≥N)
            n≥N→yn-a<ε = λ {n} → λ n≥N → proj₁ (lemma-triIneq-1-2-2 (y' n +' (-' a')) ε∈F (n≥N→∣yn-a∣<ε (n≥N→n≥Ny₁ n≥N)))
            ...
            n≥N→∣zn-a∣<ε = λ {n} → λ n≥N → lemma-triIneq-2-1' (z' n +' (-' a')) ε∈F (n≥N→zn-a<ε n≥N) (n≥N→-ε<zn-a n≥N)
  postulate
  ...
  LeastUpperBoundProperty : Set₁
  LeastUpperBoundProperty = {S : A → Set} → BoundedAbove S
                          → Σ[ b ∈ A ] Σ[ b' ∈ F b ]
                            (b' ≈sup S)
  ...
  postulate
    Theorem1 : MonotoneSequeceProperty ↔ CauchyComplete
    Theorem2 : CauchyComplete ↔ LeastUpperBoundProperty
    Theorem3 : LeastUpperBoundProperty ↔ MonotoneSequeceProperty
